...
//보통 Run이라고 생성하는 클래스는 객체생성을 위한 class가 아니라 main문을  통해 프로그램 실행을 위한 클래스다.
public class Run {
	
		/*
		 1. 객체지향 언어 : "객체"를 "지향"하는 언어로 "객체중심"으로 돌아간다.
		 
		 2. 객체란 : 현실세계에서 독립적(목적이 있고 가치가 이쓴 것)으로 존재하는 모든 것을 의미
		 		 	현실세계에서는 객체들간의 상호작요으로 돌아감
		 ...
		 4) 최종적으로 추려진 속성들을 어떤 자료형으로, 어떤 변수형으로 사용할 것인지 정의
		 
		 ex) 학생 관리 프로그램
		 1) 학생관련 객체(이름...)
		 2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 잔다, 화장실간다, 공부한다, 휴식한다)
		 3) 학생 "인적사항"을 관리히는 프로그램 : 이름, 나이, 주소, 전화번호, 반....
		...
		6. 추상화한 객체들 프로그램에 적용시키기
			1. "변수"만으로 프로그래밍을 한다.
	 		...
	 		3. 그래서 나온 개념 -> 구조체
	 			...
	 			-> 자바에서는 구조체개냠을 좀 더 확장해서 클래스 라는 개념을 만들었다.
	 			
	 			캡슐화 : 추상화를 통해서 정의된 속성들과 기능들을 하나로 묶ㅇ어 관리하는 방법
	 			클래스에서 가장 중요한 목적인 "데이터의 접근제한(정보은닉)"을 원칙으로
	 			외부로부터의 "데이터접근을 막고"
	 			대신에 "데이터를 간접적으로 처리(값을 변경하거나, 가져오는 것등)"할 메소드들을 클래스 내부에 작성하야 관리하는것
	 			다양한 기능을 묶어 하나의 큰 기능을 만들어 주는것
	 */
