...
//		System.out.println(dArr.hashCode());
		 ...
		 * 주소값을 담고 있는 변수는 참조변수(레퍼런스 뱐수)라고 표현
		 * 
		 * 기본자료(원시타입) : boolean, int, char,byte, long, double등등으로 선언된 변수
		 * -> 실제 리리터러값을 바로 담을 수 있다.
		 * 
		 * 그외 자료형(String, Scanner, int[], doduble[], char[]...) -> 참조변수
		 */
		...
		// => 공간이 만들어질 때 JVM 기본값으로라도 초기화를 진행한다
		...
		/*
		 * null을 가지고 있는 참조변수에 잡근하여 다른 정보를 구하고자 하면 어떻게 될까?
		 * 항상 오류가 발생한다.
		...
		arr = null ;
		// 배열을 강제로 삭제시키고자 한다면 ->견결고리를 끊어주면 된다(null)대입)
		
		int[] arr2 = {1,1,1,1,1};
		int[] arr3 = {1,1,1,1,1};
		System.out.println(arr2 == arr3); //false -> 각참조변수는 ==0을 통해서 비교시 주소값을 비교하기 떄문에
